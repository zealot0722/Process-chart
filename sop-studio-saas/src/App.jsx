...
import {
  ...
  GitPullRequest, PlusCircle, Search, ArrowUp, ArrowDown, IndentDecrease, IndentIncrease, Copy
} from 'lucide-react';
                         ...
                         <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-lg shadow-xl z-50">
                           <div className="flex items-center justify-between px-3 py-2 border-b border-slate-100 text-xs text-slate-500">
                             ...
                             <div className="flex items-center gap-1">
                               ...
                               <button onClick={(e) => { e.stopPropagation(); setShowLinkPicker(false); }} className="text-slate-400 hover:text-slate-600">
                                 <X size={12} />
                               </button>
                             </div>
                           </div>
              ...
              {pages.map(page => (
                ...
                {isEditable && activePageId === page.id && (
                  <div className="flex gap-1 ml-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={(e) => { e.stopPropagation(); editPageName(page.id); }} className="hover:text-blue-600"><Edit2 size={12} /></button>
                    <button onClick={(e) => { e.stopPropagation(); duplicatePage(page.id); }} className="hover:text-emerald-600"><Copy size={12} /></button>
                    <button onClick={(e) => { e.stopPropagation(); deletePage(page.id); }} className="hover:text-red-600"><X size={12} /></button>
                  </div>
                )}
                </div>
              ))}
